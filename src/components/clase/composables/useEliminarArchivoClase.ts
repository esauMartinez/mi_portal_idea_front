...
import { deleteArchivoClase } from '../services/clase'
import { mensajeError, mensajeOk, pregunta } from '@/helper/mensajes'

const useEliminarArchivoClase = () => {
  const queryClient = useQueryClient()

  const eliminarMutation = useMutation({
    mutationKey: ['deleteArchivoClase'],
    mutationFn: deleteArchivoClase,
    onMutate: () => {
...
export default useEliminarArchivoClase
